                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (!CheckInputs(tx, state, view, true, scriptcontext, scriptVerifyFlags, true, txdata)) {
            ...
            if (!tx.HasWitness() && CheckInputs(tx, stateDummy, view, true, scriptcontext, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, txdata) &&
                !CheckInputs(tx, stateDummy, view, true, scriptcontext, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, txdata)) {
        ...
        if (!CheckInputs(tx, state, view, true, scriptcontext, MANDATORY_SCRIPT_VERIFY_FLAGS, true, txdata))
        {
...
bool CScriptCheck::operator()() {
    ...
    return VerifyScript(scriptSig, scriptPubKey, witness, nFlags, CachingTransactionSignatureChecker(ptxTo, nIn, amount, *pscriptcontext, cacheStore, *txdata), &error);
}
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, const CScriptContext& scriptcontext, unsigned int flags, bool cacheStore, PrecomputedTransactionData& txdata, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheck check(*coins, tx, i, scriptcontext, flags, cacheStore, &txdata);
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_CONTEXTUAL_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        // non-upgraded nodes.
                        CScriptCheck check2(*coins, tx, i, scriptcontext,
