                                         ...
                                         PackageValidationState& package_state)
{
    ...
    if (!m_pool.CheckPackageLimits(txns, all_tx_vsizes, limits, err_string)) {
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactions(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    std::vector<int64_t> all_tx_vsizes;
    all_tx_vsizes.reserve(all_tx_vsizes.size());
    std::transform(workspaces.cbegin(), workspaces.cend(), std::back_inserter(all_tx_vsizes),
                   [](const auto& ws) { return ws.m_vsize; });

    // Apply package mempool ancestor/descendant limits. Skip if there is only one transaction,
    // because it's unnecessary. Also, CPFP carve out can increase the limit for individual
    // transactions, but this exemption is not extended to packages in CheckPackageLimits().
    std::string err_string;
    if (txns.size() > 1 && !PackageMempoolChecks(args, txns, all_tx_vsizes, package_state)) {
        return PackageMempoolAcceptResult(package_state, std::move(results));
    }
