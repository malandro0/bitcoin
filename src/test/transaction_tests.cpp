	    ...
	    for (unsigned int inpIdx = 0; inpIdx < inputs.size(); inpIdx++) {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                BOOST_CHECK_MESSAGE(VerifyScript(tx.vin[i].scriptSig, mapprevOutScriptPubKeys[tx.vin[i].prevout],
                                                 witness, verify_flags, TransactionSignatureChecker(&tx, i, amount, NullScriptContext, txdata), &err),
                                    strTest);
...
void CheckWithFlag(const CTransactionRef& output, const CMutableTransaction& input, int flags, bool success)
{
    ...
    bool ret = VerifyScript(inputi.vin[0].scriptSig, output->vout[0].scriptPubKey, &inputi.vin[0].scriptWitness, flags, TransactionSignatureChecker(&inputi, 0, output->vout[0].nValue, NullScriptContext), &error);
    assert(ret == success);
}
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
    ...
    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        ...
        CScriptCheck check(coins, tx, i, NullScriptContext, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, false, &txdata);
