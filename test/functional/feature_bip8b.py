...
class BIP8Test(BitcoinTestFramework):
    ...
    def apply_blocks_and_check(self, versions, base_height, state, height=None):
        ...
        # always active
        assert_equal(status[1], {'type': 'bip8', 'bip8': {'status': 'active', 'startheight': -1, 'timeoutheight': 0, 'min_activation_height': 0, 'lockinontimeout': True, 'since': 0}, 'height': 0, 'active': True})
        ...
        if state[0] == "defined":
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'defined', 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': False, 'since': state[1]}, 'active': False})
        elif state[0] == "started" or state[0] == "must_signal":
            since = state[1] if state[0] == "started" else (state[1] - 288)
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'started', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': False, 'since': since, 'statistics': None}, 'active': False})
        elif state[0] == "locked_in":
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'locked_in', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': False, 'since': state[1], 'statistics': None}, 'active': False})
        elif state[0] == "active":
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'active', 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': False, 'since': state[1]}, 'height': state[1], 'active': True})
        elif state[0] == "stopped":
            failat = state[1] - (state[1] % 144) + 144
            assert_equal(status[2], {'type': 'bip8', 'bip8': {'status': 'failed', 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': False, 'since': failat}, 'active': False})
        else:
            assert False, ("bad state %r" % (state))

        # lockinontimeout=true
        if "statistics" in status[3]["bip8"]:
            if state[0] == 'locked_in':
                assert('possible' not in status[3]["bip8"]["statistics"])
            else:
                assert_equal(status[3]["bip8"]["statistics"]["possible"], True)
            status[3]["bip8"]["statistics"] = None

        if state[0] == "defined":
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'defined', 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': True, 'since': state[1]}, 'active': False})
        elif state[0] == "started":
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'started', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': True, 'since': state[1], 'statistics': None}, 'active': False})
        elif state[0] == "must_signal" or state[0] == "stopped":
            since = state[1] - state[1] % 144
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'must_signal', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': True, 'since': since, 'statistics': None}, 'active': False})
        elif state[0] == "locked_in":
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'locked_in', 'bit': 28, 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': True, 'since': state[1], 'statistics': None}, 'active': False})
        elif state[0] == "active":
            assert_equal(status[3], {'type': 'bip8', 'bip8': {'status': 'active', 'startheight': start, 'timeoutheight': stop, 'min_activation_height': 0, 'lockinontimeout': True, 'since': state[1]}, 'height': state[1], 'active': True})
        else:
            assert False, ("bad state %r" % (state))
